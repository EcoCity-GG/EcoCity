...
import { MapPin, Recycle, TreeDeciduous, Calendar, Leaf, Lamp, Droplets, Maximize, Minimize } from 'lucide-react';
...
const EcoMap = ({ hideControls = false, eventMode = false, searchQuery = '', fullScreen = false, nonInteractive = false, previewMode = false }: EcoMapProps) => {
  ...
  useEffect(() => {
    ...
    filteredPoints.forEach(point => {
      if (typeof window !== 'undefined' && window.L) {
        ...
        // Adicionar eventos de clique baseado na interatividade
        const shouldAddClickEvents = previewMode ? !isMobile : !nonInteractive;
        if (shouldAddClickEvents) {
          marker.on('click', () => {
  ...
  }, [map, isMapInitialized, mapPoints, filter, effectiveSearchQuery, eventMode, events, nonInteractive, previewMode, isMobile]);
  ...
  const getMarkerIconHtml = (type: string) => {
    ...
    switch (type) {
      ...
      case 'plant-sales':
        ...
        iconSvg = `<svg fill="#ffffff" width="16" height="16" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>`;
        break;
      case 'lamp-collection':
      ...
      case 'oil-collection':
        ...
        iconSvg = `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M7 16.3c2.2 0 4-1.83 4-4.05 0-1.16-.57-2.26-1.71-3.19S7.29 6.75 7 5.3c-.29 1.45-1.14 2.84-2.29 3.76S3 11.1 3 12.25c0 2.22 1.8 4.05 4 4.05z"/><path d="M12.56 6.6A10.97 10.97 0 0 0 14 3.02c.5 2.5 2.04 4.6 4.14 5.93s3.73 3.15 3.73 5.25a6.5 6.5 0 0 1-13 0c0-1.7.83-3.2 2.13-4.2z"/></svg>`;
        break;
      default:
        bgColor = 'bg-eco-green';
        iconSvg = `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z"/><circle cx="12" cy="10" r="3"/></svg>`;
  ...
  const typeInfo = {
    ...
    'oil-collection': { label: 'Coleta de Óleo', color: 'bg-orange-500', description: 'Pontos de coleta para descarte correto de óleo de cozinha.' }
  };
